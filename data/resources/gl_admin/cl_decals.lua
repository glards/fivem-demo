...
RegisterCommand('decal', function (src, arg, rawInput)
    ...
    local decalType = tonumber(arg[1]) or 1030
        ...
        if enableMouseDecal then
            ...
            if hit ~= 0 then -- Draw a preview using "DrawMarker":
                ...
                if IsControlJustReleased(0, 51) then -- ~INPUT_CONTEXT~
                    ...
                    local lastNormal = string.format("%2f %2f %2f", surface.x,  surface.y, surface.z)
                    local lastPos = string.format("%2f %2f %2f", pos.x,  pos.y, pos.z)
                    print("decal location", string.format("POS: %s~n~NORMAL: %s",
                        lastPos,
                        lastNormal)
                    )
...
RegisterCommand('toggleDecal', function (src, arg, rawInput)
    enableMouseDecal = not enableMouseDecal
    print("enableMouseDecal", enableMouseDecal)
end)
